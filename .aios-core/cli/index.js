...
function tryLoadCommand(factoryPath, factoryName) {
  try {
...
`);
  ...
  // Metrics is optional in lightweight distributions where quality modules may be absent.
  const metricsCommand = tryLoadCommand('./commands/metrics', 'createMetricsCommand');
  if (metricsCommand) {
